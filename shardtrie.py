...
def get_timestamp():
    ...
    return "%d/%.2d/%.2d %.2d:%.2d:%.2d:%d" % (d.year, d.month, d.day, d.hour,
                                               d.minute, d.second,
                                               d.microsecond)
...
class Node():
    ...
    def set_distributed(self):
        ...
        new_node.data = {'timestamp': get_timestamp(),
                         'data': None,
...
if __name__ == "__main__":
    ...
    sub = trie.get_large_subtries()
    new_trie = trie.split_trie(sub[0][-1].key)
    sub2 = trie.get_large_subtries()
    try:
        trie.get_node('object1')
    except ShardTrieDistributedBranchException as ex:
        print ex

    trie.join_subtrie(new_trie)
    sub3 = trie.get_large_subtries()
    import pprint
    pprint.pprint(trie.dump())
