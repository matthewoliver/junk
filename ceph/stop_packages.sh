...
do_umountall() {
    ...
    VSTART_IP_PORTS=$(ceph -c $conf_fn mon metadata 2>/dev/null | jq -j '.[].addrs')
    ...
    CEPH_FUSE_MNTS=$(ceph -c $conf_fn tell mds.* client ls 2>/dev/null | grep mount_point | tr -d '",' | awk '{print $2}')
...
if [ $stop_all -eq 1 ]; then
    if ceph -s --connect-timeout 1 -c $conf_fn >/dev/null 2>&1; then
        # Umount mounted filesystems from vstart cluster
        do_umountall
    fi

    if rbd device list -c $conf_fn >/dev/null 2>&1; then
        rbd device list -c $conf_fn | tail -n +2 |
        while read DEV; do
            ...
            sudo rbd device unmap "${DEV}" -c $conf_fn
        done

        if [ -n "$(rbd device list -c $conf_fn)" ]; then
